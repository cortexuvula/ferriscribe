//! Vocabulary CRUD HTTP API for paired clients.
//!
//! Lives on the office server alongside the sharing service. Bearer-validated
//! against the same `TokenStore` the auth proxy uses, so any client whose
//! pairing has been revoked is rejected here too.
//!
//! Routes (all under `/v1/vocabulary`):
//!   GET    /                       — list all entries (or by category via ?category=)
//!   GET    /count                  — (total, enabled)
//!   POST   /                       — insert a new entry
//!   PUT    /:id                    — replace an entry by uuid
//!   DELETE /:id                    — delete one entry
//!   DELETE /                       — delete all entries
//!
//! Wire format: same shape as the existing `VocabularyEntry` serde
//! definition, so the client can deserialize directly into the type it
//! already uses for the local DB.
//!
//! No PHI passes through these routes — vocabulary is clinic-defined
//! correction terms, not patient content — so logging request paths and
//! status codes is safe.
...
use medical_db::{Database, vocabulary::VocabularyRepo};
