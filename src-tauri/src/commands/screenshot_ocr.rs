...
async fn capture_ocr_inner(
    ...
    // provider stack as document OCR, and runs the shared echo cleaner
    // (`clean_ocr_text`) on the response before returning. No new network
    // surface.
...
mod tests {
    ...
    fn clean_ocr_text_real_sample_harness() {
        use medical_processing::ocr::clean_ocr_text;
