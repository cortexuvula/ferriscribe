...
- **Remaining copy-paste clusters** (2026-08-17 review; extended by the 2026-09-02 tech-debt review) — RecordTab/GenerateTab patient-context state (`usePatientContextFields()` composable), ContextPanel/PatientContextSidebar parallel surfaces. Added 2026-09-02: the provider host/port/test-connection pattern exists **five times** (Models.svelte's three provider sections, `SttRemoteSection.svelte`, `StepProvider.svelte`) plus a 3-way copy in `endpointHealth.probeAi` — extract a `ProviderServerSection.svelte` + `useTestConnection()` composable; the generation commands hand-copy the shared driver's scaffolding (~250 lines: soap/peer_discussion/synopsis vs `generate_from_soap` in `generation/helpers.rs`, plus seven near-identical endpoint-offline tests); `build_sparse_fields` exists in both `commands/content_sync.rs` and `sharing_vocab_api/content_sync.rs` (~92 lines each, already diverged in signature — LWW desync risk); the vocab API repeats its authorize→spawn_blocking→repo handler scaffold ~23 times (~250 lines).
