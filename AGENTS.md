...
- **Remaining bug-review items (2026-08-17; updated by the 2026-09-02 bug review)** — `selectRecording` staleness race (no request token, last-write-wins); mid-recording crash leaves row-less plaintext WAVs invisible to sweeps (import-path half fixed 2026-09-02 — imports now flag before encrypting; capture-path crash window remains); vocab API binds `0.0.0.0` plain HTTP (Tailscale rule is client-side only); `ConditionChipsRepo::add` resets `use_count` on resurrection; deferred transactions don't serialize MAX-then-INSERT (`SQLITE_BUSY_SNAPSHOT` spurious failures — consider `BEGIN IMMEDIATE`; no `TransactionBehavior::Immediate` anywhere in `crates/db` as of 2026-09-02); `stop_recording` releases `recording_active` before consuming `current_recording`. Resolved by the 2026-09-02 fix batch: duplicate `recording-updated` clobbering (edit-protection set in the recordings store, maintained by EditorTab), chat-stream cross-contamination (stream_id on every payload + `chat_cancel_stream` + supersede-cancel), audio-path LWW stamp inflation (`update_audio_location` preserves `updated_at`), stop/import encryption-failure flag handling. Still open from that review: whole-row snapshot persists during transcription/SOAP revert concurrent editor edits (needs targeted field writes or a merge-on-persist); pull cursor unencoded `+00:00`; dictionary/chips merge snapshot outside transaction + no server merge lock; audio PUT orphaned ciphertext/tombstone misclassification; context-template whole-blob RMW; SSE subscriber dead-token 401-loop; GenerateTab unconditional re-select; Models.svelte stale cross-provider model + no fetch token; usePairing port range gaps; ContentSync.svelte missing disposed guard; RecordTab stale-id pipeline launch; ChatTab misleading clear-on-leave copy; mixed-format timestamps in `changed_since`/merge row-bump; mDNS advertiser resurrection; whisper supervisor port-8080 trust; auth-proxy POST retry/bearer-case/fail-limiter gaps.
