...
- **Pdfium download is unverified** (from the 2026-08-17 review) — `crates/processing/src/ocr/mod.rs` downloads and dlopen's a native library with no SHA-256 pin and no timeout, unlike the hash-verified whisper path in `crates/sharing/src/whisper_supervisor.rs`. Consolidate onto one shared verified-download helper before touching this path.
- **`state.rs::initialize()` god-function + zero-test command files** (2026-08-17 review) — `content_sync.rs`, `recovery.rs`, `recordings.rs`, `sharing/lifecycle.rs` commands have no shell-side tests; the inline retention sweeper in `initialize()` is untestable as embedded. Split before extending.
- **Copy-paste clusters** (2026-08-17 review) — Ollama/LM Studio endpoint-resolution (`ollama.rs`/`lmstudio.rs`), `ClientPair.svelte`/`StepPair.svelte` pairing logic, config-load `spawn_blocking` closure (~15×) and test-connection trio in `commands/providers.rs`.
