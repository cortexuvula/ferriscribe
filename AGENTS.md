...
- **Sync/merge architectural fixes needed (2026-08-17 bug review)** — in priority order: (1) tombstone-vs-live has no timestamp comparison in `content_sync.rs merge_incoming` → restores never survive sync; (2) chips/dictionary tombstones never travel server→client (`list_active`-only endpoints) → deletions resurrect after the 30-day prune; (3) server purge + client re-insert resurrects deleted recordings practice-wide; (4) writers using `RecordingsRepo::update` without revision upserts silently lose Equal-tie merges (regenerated SOAP/referral diverges across machines); (5) one future timestamp (clock skew) pins pull cursors fleet-wide; (6) audio uploads capped at `take(10)` per push batch with no retry queue (`pending_audio_uploads` in m015 is dead code).
