...
- Server sync/list endpoints (`sharing_vocab_api/condition_chips.rs`, `user_dictionary.rs`) switch from `list_active` to full-list-with-tombstones. Both repos' `merge_incoming` implementations already handle tombstones (tie-break: tombstone wins; `use_count` MAX), so old chips clients are unaffected and the tie-break becomes reachable. Tombstones are small (a condition label / dictionary word + timestamps) — full lists remain tiny.
- Dictionary client gains the missing client-side merge: `user_dict_list` and `sync_user_dictionary_cmd` call `UserDictionaryRepo::merge_incoming` with the server's full list (mirroring the chips path) before returning the active list. This also fixes the "words from other machines invisible offline / deletions never apply locally" finding.
...
- **Behavior change (needs sign-off):** `increment_use` no longer resurrects a tombstoned chip — a click on a stale list currently undeletes the condition practice-wide once tombstones propagate. Explicit add still resurrects (existing documented behavior); the fresh-install self-healing path (no row at all → create with count 1) is preserved. Under LWW a resurrection-by-click would now carry a fresh timestamp and *legitimately* win — we choose not to let a click be that powerful.
