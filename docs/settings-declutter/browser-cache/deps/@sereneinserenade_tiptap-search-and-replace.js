import { a as PluginKey, i as Plugin } from "./dist-D4JOGMZD.js";
import "./@tiptap_pm_state.js";
import { Decoration, DecorationSet } from "./@tiptap_pm_view.js";
import { r as Extension } from "./dist-COfpNBQi.js";
//#region ../../node_modules/@sereneinserenade/tiptap-search-and-replace/dist/index.js
...
function processSearches(doc, searchTerm, searchResultClass, resultIndex) {
	...
	doc === null || doc === void 0 || doc.descendants((node, pos) => {
		if (node.isText) {
			if (textNodesWithPosition[index]) textNodesWithPosition[index] = {
				text: textNodesWithPosition[index].text + node.text,
				pos: textNodesWithPosition[index].pos
			};
			else textNodesWithPosition[index] = {
				text: `${node.text}`,
				pos
			};
		} else index += 1;
	...
	for (const element of textNodesWithPosition) {
		...
		for (const m of matches) {
			if (m[0] === "") break;
			if (m.index !== void 0) results.push({
				from: pos + m.index,
	...
	for (let i = 0; i < results.length; i += 1) {
		...
		const decoration = Decoration.inline(r.from, r.to, { class: className });
...
var searchAndReplacePluginKey = new PluginKey("searchAndReplacePlugin");
var SearchAndReplace = Extension.create({
	...
	addCommands() {
		return {
			...
			nextSearchResult: () => ({ editor }) => {
				...
				if (results[nextIndex]) editor.storage.searchAndReplace.resultIndex = nextIndex;
				else editor.storage.searchAndReplace.resultIndex = 0;
				return false;
			},
			previousSearchResult: () => ({ editor }) => {
				const { results, resultIndex } = editor.storage.searchAndReplace;
				const prevIndex = resultIndex - 1;
				if (results[prevIndex]) editor.storage.searchAndReplace.resultIndex = prevIndex;
				else editor.storage.searchAndReplace.resultIndex = results.length - 1;
				return false;
			},
			replace: () => ({ editor, state, dispatch }) => {
				const { replaceTerm, results } = editor.storage.searchAndReplace;
				replace(replaceTerm, results, {
					state,
					dispatch
				});
				return false;
			...
			replaceAll: () => ({ editor, tr, dispatch }) => {
				const { replaceTerm, results } = editor.storage.searchAndReplace;
				replaceAll(replaceTerm, results, {
					tr,
					dispatch
				});
				return false;
		...
		return [new Plugin({
			...
			state: {
				...
				apply({ doc, docChanged }, oldState) {
					...
					if (!docChanged && lastSearchTerm === searchTerm && lastCaseSensitive === caseSensitive && lastResultIndex === resultIndex) return oldState;
