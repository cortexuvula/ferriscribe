...
//#endregion
//#region ../../node_modules/nspell/lib/util/rule-codes.js
var require_rule_codes = /* @__PURE__ */ __commonJSMin(((exports, module) => {
...
//#endregion
//#region ../../node_modules/nspell/lib/util/affix.js
var require_affix = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	function affix(doc) {
		...
		while (++index < lines.length) {
			...
			if (ruleType === "REP") {
				...
				while (++index <= count) {
					...
					while (++position < rule.length) compoundRuleCodes[rule.charAt(position)] = [];
				}
				index--;
				...
				while (++index <= count) {
					...
					if (add && add[0] !== "0") entry.add = add[0];
					try {
						if (remove !== "0") entry.remove = ruleType === "SFX" ? end(remove) : remove;
						if (source && source !== ".") entry.match = ruleType === "SFX" ? end(source) : start(source);
					} catch (_) {
						entry = null;
					}
					if (entry) rule.entries.push(entry);
				}
				index--;
				...
				while (++offset < source.length) {
					...
					if (character.toLowerCase() === character) value.push(character);
				}
				offset = -1;
				while (++offset < alphabet.length) if (source.indexOf(alphabet[offset]) < 0) value.push(alphabet[offset]);
				flags[ruleType] = value;
			} else if (ruleType === "KEY") push.apply(flags[ruleType], parts[1].split("|"));
			else if (ruleType === "COMPOUNDMIN") flags[ruleType] = Number(parts[1]);
			else if (ruleType === "ONLYINCOMPOUND") {
				flags[ruleType] = parts[1];
				compoundRuleCodes[parts[1]] = [];
			} else if (ruleType === "FLAG" || ruleType === "KEEPCASE" || ruleType === "NOSUGGEST" || ruleType === "WORDCHARS") flags[ruleType] = parts[1];
			else flags[ruleType] = parts[1];
		}
		if (isNaN(flags.COMPOUNDMIN)) flags.COMPOUNDMIN = 3;
		if (!flags.KEY.length) flags.KEY = defaultKeyboardLayout;
		/* istanbul ignore if - Dictionaries seem to always have this. */
		if (!flags.TRY) flags.TRY = alphabet.concat();
		if (!flags.KEEPCASE) flags.KEEPCASE = false;
		return {
			compoundRuleCodes,
			replacementTable,
			conversion,
		...
		function pushLine(line) {
			line = line.trim();
			if (line && line.charCodeAt(0) !== 35) lines.push(line);
		}
	}
	function end(source) {
...
//#endregion
//#region ../../node_modules/nspell/lib/util/normalize.js
var require_normalize = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	module.exports = normalize;
	function normalize(value, patterns) {
		var index = -1;
		while (++index < patterns.length) value = value.replace(patterns[index][0], patterns[index][1]);
		return value;
	}
}));
//#endregion
//#region ../../node_modules/nspell/lib/util/flag.js
var require_flag = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	module.exports = flag;
...
//#endregion
//#region ../../node_modules/nspell/lib/util/exact.js
var require_exact = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	var flag = require_flag();
	module.exports = exact;
	function exact(context, value) {
		var index = -1;
		if (context.data[value]) return !flag(context.flags, "ONLYINCOMPOUND", context.data[value]);
		if (value.length >= context.flags.COMPOUNDMIN) {
			while (++index < context.compoundRules.length) if (context.compoundRules[index].test(value)) return true;
		}
		return false;
	}
}));
//#endregion
//#region ../../node_modules/nspell/lib/util/form.js
var require_form = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	function form(context, value, all) {
		...
		if (exact(context, normal)) {
			if (!all && flag(context.flags, "FORBIDDENWORD", context.data[normal])) return null;
			return normal;
		}
		if (normal.toUpperCase() === normal) {
			alternative = normal.charAt(0) + normal.slice(1).toLowerCase();
			if (ignore(context.flags, context.data[alternative], all)) return null;
			if (exact(context, alternative)) return alternative;
		}
...
//#endregion
//#region ../../node_modules/nspell/lib/correct.js
var require_correct = /* @__PURE__ */ __commonJSMin(((exports, module) => {
...
//#endregion
//#region ../../node_modules/nspell/lib/util/casing.js
var require_casing = /* @__PURE__ */ __commonJSMin(((exports, module) => {
...
//#endregion
//#region ../../node_modules/nspell/lib/suggest.js
var require_suggest = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	function suggest(value) {
		...
		while (++index < value.length) {
			...
			while (++offset < self.flags.KEY.length) {
				...
				otherOffset = -1;
				while (++otherOffset < group.length) if (otherOffset !== position) {
		...
		if (value === replacement || currentCase === null) values.push(value.charAt(0).toUpperCase() + replacement.slice(1));
	...
	function generate(context, memory, words, edits) {
		...
		var inject;
		var offset;
		if (edits) while (++index < edits.length) check(edits[index], true);
		index = -1;
...
//#endregion
//#region ../../node_modules/nspell/lib/spell.js
var require_spell = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	var form = require_form();
	var flag = require_flag();
...
//#endregion
//#region ../../node_modules/nspell/lib/util/apply.js
	...
	function apply(value, rule, rules, words) {
		...
		while (++index < rule.entries.length) {
			...
			if (!entry.match || entry.match.test(value)) {
				...
				words.push(next);
				if (continuation && continuation.length) while (++position < continuation.length) {
					continuationRule = rules[continuation[position]];
					if (continuationRule) apply(next, continuationRule, rules, words);
				}
			}
		}
		return words;
...
//#endregion
//#region ../../node_modules/nspell/lib/util/add.js
var require_add$1 = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	function addRules(dict, word, rules) {
		var curr = dict[word];
		if (word in dict) {
			if (curr === NO_RULES) dict[word] = rules.concat();
			else push.apply(curr, rules);
		} else dict[word] = rules.concat();
	}
	function add(dict, word, codes, options) {
		...
		var newWords;
		var otherNewWords;
		if (!("NEEDAFFIX" in options.flags) || codes.indexOf(options.flags.NEEDAFFIX) < 0) addRules(dict, word, codes);
		while (++position < codes.length) {
			rule = options.rules[codes[position]];
			if (codes[position] in options.compoundRuleCodes) options.compoundRuleCodes[codes[position]].push(word);
			if (rule) {
				newWords = apply(word, rule, options.rules, []);
				offset = -1;
				while (++offset < newWords.length) {
					if (!(newWords[offset] in dict)) dict[newWords[offset]] = NO_RULES;
					if (rule.combineable) {
						...
						while (++subposition < codes.length) {
							...
							if (combined && combined.combineable && rule.type !== combined.type) {
								...
								suboffset = -1;
								while (++suboffset < otherNewWords.length) if (!(otherNewWords[suboffset] in dict)) dict[otherNewWords[suboffset]] = NO_RULES;
							}
						}
					}
				}
...
//#endregion
//#region ../../node_modules/nspell/lib/add.js
var require_add = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	var push = require_add$1();
	module.exports = add;
	var NO_CODES = [];
...
//#endregion
//#region ../../node_modules/nspell/lib/remove.js
...
//#endregion
//#region ../../node_modules/nspell/lib/word-characters.js
var require_word_characters = /* @__PURE__ */ __commonJSMin(((exports, module) => {
...
//#endregion
//#region ../../node_modules/nspell/lib/util/dictionary.js
var require_dictionary$1 = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	var parseCodes = require_rule_codes();
	var add = require_add$1();
	...
	function parse(buf, options, dict) {
		...
		while (index > -1) {
			if (value.charCodeAt(last) !== 9) parseLine(value.slice(last, index), options, dict);
			last = index + 1;
			index = value.indexOf("\n", last);
		}
	...
	function parseLine(line, options, dict) {
		...
		if (hashOffset > -1) {
			...
			} else word = line.slice(0, hashOffset);
		} else if (slashOffset > -1) {
			word = line.slice(0, slashOffset);
			codes = line.slice(slashOffset + 1);
		} else word = line;
		word = word.trim();
		if (word) add(dict, word, parseCodes(options.flags, codes.trim()), options);
	}
}));
//#endregion
//#region ../../node_modules/nspell/lib/dictionary.js
var require_dictionary = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	var parse = require_dictionary$1();
	module.exports = add;
	function add(buf) {
		var self = this;
		var index = -1;
...
//#endregion
//#region ../../node_modules/nspell/lib/personal.js
var require_personal = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	module.exports = add;
	function add(buf) {
		var self = this;
		...
		while (++index < lines.length) {
			...
			if (forbidden) self.data[word].push(flag);
		}
		return self;
	}
}));
//#endregion
//#region ../../node_modules/nspell/lib/index.js
var require_lib = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	function NSpell(aff, dic) {
		...
		if (dictionaries) {
			while (++index < dictionaries.length) if (dictionaries[index].dic) this.dictionary(dictionaries[index].dic);
		}
	}
}));
//#endregion
