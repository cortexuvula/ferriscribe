<script lang="ts">
  ...
  async function act(id: string, action: 'promote' | 'reject' | 'unpromote' | 'restore') {
    const new_status =
      action === 'restore' || action === 'unpromote' ? 'candidate' :
  ...
  function onKey(ev: KeyboardEvent) {
    ...
    else if (key === 'r') { act(items[cursorIndex].id, 'restore'); ev.preventDefault(); }
  }
...
<div class="rejected-list">
  ...
  {#if !loading && items.length === 0}
    <div class="empty">No rejected candidates.</div>
  {/if}

  {#each items as g, i (g.id)}
    <div class:cursor-row={i === cursorIndex} class="row-wrap">
      <GenerationCard generation={g} mode="rejected" onAction={act} />
    </div>
  {/each}
  ...
  <p class="kbd-hint">
    <kbd>J</kbd>/<kbd>K</kbd> navigate · <kbd>R</kbd> restore
  </p>
</div>

<style>
  .rejected-list { display: flex; flex-direction: column; gap: 0.25rem; }
